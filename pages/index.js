...
const [image, setImage] = useState('/Normal case (213).jpg');
  ...
  const loadModel = async () => {
    const model = await tf.loadLayersModel("https://mri-lung-detection.s3.us-east.cloud-object-storage.appdomain.cloud/model.json");
    setModel(model)
    model.summary();
  };
  ...
  const handleChange = async (event) => {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      await tf.tidy(() => {
        ...
        const tensorImg = tf.browser.fromPixels(image,1).resizeNearestNeighbor([256, 256]).toFloat().expandDims();
      ...
      <div className="mb-32 grid text-center lg:mb-0 lg:grid-cols-4 lg:text-left">
        ...
        >
          ...
          <p className={`m-0 max-w-[30ch] text-sm opacity-50`}>
           Explore this jupiter notebook to undestand how this model was built.
          </p>
        </a>
        ...
        >
          ...
          <p className={`m-0 max-w-[30ch] text-sm opacity-50`}>
            Learn about tensorflow and tensorflow.js required to build deep learning models.
          </p>
        </a>
